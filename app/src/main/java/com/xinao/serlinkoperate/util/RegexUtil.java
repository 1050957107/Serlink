...
public final class RegexUtil {
     ...
     * 中文名
     *
     * @param name 只能是中文，长度为2-12位
     * @return 验证成功返回true，验证失败返回false
     */
    public static boolean IsChinese12(String name) {
        String regex = "^[\u4e00-\u9fa5]{2,12}";
        return Pattern.matches(regex, name);
    }

    /**
     * 中文名 长度 2-7 或者英文名 汉字与字母不可共存
     *
     * @param name
     * @return
     */
     ...
     * 纳税人识别号15位
     */
    public static boolean IsNumber15(String card) {
        String regex = "^\\d{15}$";
        return Pattern.matches(regex, card);
    }
     ...
     * 判断输入 数字 字母 大写 4位
     */
    public static boolean checkNumberAndzifuBig(String character) {
     ...
     * @return
     */
    public static boolean checkText1(String character) {
        String regex = "[`~!@#$^&*()=|{}':;',\\\\[\\\\].<>/?~！@#￥……&*（）——|{}【】‘；：”“'。，、？]";
        return Pattern.matches(regex, character);
    }
    ...
    /**
     * 得到控件上的文字信息 比如 TextView
     *
     * @param view 对应的控件
     * @return
     */
    public static String textToString(View view) {
        if (view instanceof TextView) {
            return ((TextView) view).getText().toString().trim();
        } else if (view instanceof AutoCompleteTextView) {
            return ((AutoCompleteTextView) view).getText().toString().trim();
    ...
    public static String cidReplaceWithStar(String bankCard) {

        if (bankCard.isEmpty() || bankCard == null) {
            return null;
        } else {
            return replaceAction(bankCard, "(?<=[\\d]{5})\\d(?=[\\d]{3})");
        }
    }
